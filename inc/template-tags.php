 ...
 */
function sirrine_site_logo( $args = array(), $echo = true ) {
	...
	/**
	 * Filters the arguments for `sirrine_site_logo()`.
	 *
	 * @param array  $args     Parsed arguments.
	 * @param array  $defaults Function's default arguments.
	 */
	$args = apply_filters( 'sirrine_site_logo_args', $args, $defaults );
	 ...
	 */
	$html = apply_filters( 'sirrine_site_logo', $html, $args, $classname, $contents );
 ...
 */
function sirrine_site_description( $echo = true ) {
	 ...
	 */
	$html = apply_filters( 'sirrine_site_description', $html, $description, $wrapper );
 ...
 * @return bool
 */
function sirrine_is_comment_by_post_author( $comment = null ) {
 ...
 */
function sirrine_filter_comment_reply_link( $link ) {
...
add_filter( 'comment_reply_link', 'sirrine_filter_comment_reply_link' );
 ...
 */
function sirrine_the_post_meta( $post_id = null, $location = 'single-top' ) {

	echo sirrine_get_post_meta( $post_id, $location ); //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in sirrine_get_post_meta().
 ...
 */
function sirrine_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$text = sprintf(
		wp_kses(
			...
			__( 'Edit <span class="screen-reader-text">%s</span>', 'sirrine' ),
			array(
				'span' => array(
					'class' => array(),
				),
	...
	return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . sirrine_get_theme_svg( 'edit' ) . '</span><span class="meta-text"><a href="' . esc_url( $edit_url ) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';

}

add_filter( 'edit_post_link', 'sirrine_edit_post_link', 10, 3 );
 ...
 */
function sirrine_get_post_meta( $post_id = null, $location = 'single-top' ) {
	 ...
	 */
	$disallowed_post_types = apply_filters( 'sirrine_disallowed_post_types_for_meta_output', array( 'page' ) );
	...
	if ( 'single-top' === $location ) {
		...
		*/
		$post_meta = apply_filters(
			'sirrine_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		...
		*/
		$post_meta = apply_filters(
			'sirrine_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
	...
	if ( $post_meta && ! in_array( 'empty', $post_meta, true ) ) {
		...
		<div class="post-meta-wrapper<?php echo esc_attr( $post_meta_wrapper_classes ); ?>">
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				 ...
				 */
				do_action( 'sirrine_start_of_post_meta_list', $post_id, $post_meta, $location );
				...
				if ( in_array( 'author', $post_meta, true ) ) {
					...
					<li class="post-author meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post author', 'sirrine' ); ?></span>
							<?php sirrine_the_theme_svg( 'user' ); ?>
						</span>
				...
				if ( in_array( 'post-date', $post_meta, true ) ) {
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post date', 'sirrine' ); ?></span>
							<?php sirrine_the_theme_svg( 'calendar' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'categories', $post_meta, true ) && has_category() ) {
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Categories', 'sirrine' ); ?></span>
							<?php sirrine_the_theme_svg( 'folder' ); ?>
						</span>
						<span class="meta-text">
							<?php _ex( 'In', 'A string that is output before one or more categories', 'sirrine' ); ?> <?php the_category( ', ' ); ?>
						</span>
					</li>
					<?php
				...
				if ( in_array( 'tags', $post_meta, true ) && has_tag() ) {
					...
					<li class="post-tags meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Tags', 'sirrine' ); ?></span>
							<?php sirrine_the_theme_svg( 'tag' ); ?>
						</span>
						<span class="meta-text">
							<?php the_tags( '', ', ', '' ); ?>
						</span>
				...
				if ( in_array( 'comments', $post_meta, true ) && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					...
					<li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php sirrine_the_theme_svg( 'comment' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'sticky', $post_meta, true ) && is_sticky() ) {
					...
					<li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php sirrine_the_theme_svg( 'bookmark' ); ?>
						</span>
						<span class="meta-text">
							<?php _e( 'Sticky post', 'sirrine' ); ?>
						</span>
					</li>
					<?php
				 ...
				 *                          Accepts 'single-top' or 'single-bottom'.
				 */
				do_action( 'sirrine_end_of_post_meta_list', $post_id, $post_meta, $location );
 ...
 */
function sirrine_filter_wp_list_pages_item_classes( $css_class, $item, $depth, $args, $current_page ) {
...
add_filter( 'page_css_class', 'sirrine_filter_wp_list_pages_item_classes', 10, 5 );
 ...
 */
function sirrine_add_sub_toggles_to_main_menu( $args, $item, $depth ) {
	...
	if ( isset( $args->show_toggles ) && $args->show_toggles ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$toggle_duration      = sirrine_toggle_duration();

			// Add the sub menu toggle.
			$args->after .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'sirrine' ) . '</span>' . sirrine_get_theme_svg( 'chevron-down' ) . '</button>';
...
add_filter( 'nav_menu_item_args', 'sirrine_add_sub_toggles_to_main_menu', 10, 3 );
 ...
 */
function sirrine_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		$svg = sirrine_SVG_Icons::get_social_link_svg( $item->url );
		if ( empty( $svg ) ) {
			$svg = sirrine_get_theme_svg( 'link' );
		}
...
add_filter( 'walker_nav_menu_start_el', 'sirrine_nav_menu_social_icons', 10, 4 );
...
add_action( 'wp_head', 'sirrine_no_js_class' );
 ...
 */
function sirrine_body_classes( $classes ) {
...
add_filter( 'body_class', 'sirrine_body_classes' );
 ...
 */
function sirrine_get_the_archive_title( $title ) {

	$regex = apply_filters(
		'sirrine_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'sirrine_get_the_archive_title' );
 ...
 */
function sirrine_toggle_duration() {
	/**
	 ...
	 */
	$duration = apply_filters( 'sirrine_toggle_duration', 250 );
 ...
 */
function sirrine_unique_id( $prefix = '' ) {
	static $id_counter = 0;
