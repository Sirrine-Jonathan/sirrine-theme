...
if ( ! class_exists( 'sirrine_Walker_Page' ) ) {
	/**
	 * CUSTOM PAGE WALKER
	 * A custom walker for pages.
	 */
	class sirrine_Walker_Page extends Walker_Page {
		...
		public function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', 'sirrine' ), $page->ID );
			}
			...
			if ( isset( $args['show_toggles'] ) && true === $args['show_toggles'] ) {
				...
				if ( isset( $args['pages_with_children'][ $page->ID ] ) ) {
					...
					$args['list_item_after'] .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'sirrine' ) . '</span>' . sirrine_get_theme_svg( 'chevron-down' ) . '</button>';
